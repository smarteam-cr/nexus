...
// Colores por etapa (idénticos a LifecycleReport para consistencia visual)
...
// Etapas del embudo de conversión en orden descendente
const FUNNEL_ORDER = [
...
export default function ContactFunnelWidget({ contacts, totalContacts }: Props) {
  ...
  // Datos para KPIs del header
  const leadCount     = contacts.find((c) => c.value === "lead")?.count ?? 0;
  const mqlCount      = contacts.find((c) => c.value === "marketingqualifiedlead")?.count ?? 0;
  const customerCount = contacts.find((c) => c.value === "customer")?.count ?? 0;
  ...
  // Fila raíz: "Todos los contactos" (ancho 100%)
  const topRow = {
    value: "__total__",
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-sm font-semibold text-white">Embudo de conversión de contactos</h3>
      ...
      <p className="text-xs text-gray-600 text-right">
        Tasas calculadas sobre contactos al momento de la captura · No incluye tiempo de transición entre etapas
      </p>
    </div>
  );
}
