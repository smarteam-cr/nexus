/**
 * scripts/seed-propuesta-cse-liliana.ts
 *
 * Propuesta de contratación de **Liliana Moreno** para el puesto de Customer
 * Success Executive — documento de Roles, docType PROPUESTA, id estable
 * "propuesta-cse-liliana-v1". UPSERT idempotente.
 *
 ...
 * pisar lo que se haya editado acá desde la pantalla.
 *
 * Uso:
 *   dry-run:  npx tsx scripts/seed-propuesta-cse-liliana.ts
 *   aplicar:  $env:ALLOW_PROD_WRITE="1"; npx tsx scripts/seed-propuesta-cse-liliana.ts --apply
 */
...
async function main() {
  ...
  try {
    ...
    if (!APPLY) {
      ...
      console.log('  $env:ALLOW_PROD_WRITE="1"; npx tsx scripts/seed-propuesta-cse-liliana.ts --apply');
      return;
    }
    ...
    console.log("\n✓ Listo. Abrí Roles y revisá la propuesta de Liliana.");
  } finally {
    await close();
  }
}
