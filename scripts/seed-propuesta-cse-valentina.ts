/**
 * scripts/seed-propuesta-cse-valentina.ts
 *
 * Propuesta de contratación de **Valentina Sandoval** para el puesto de Customer
 * Success Executive — documento de Roles, docType PROPUESTA, id estable
 * "propuesta-cse-valentina-v1". UPSERT idempotente.
 *
 * Es el CLON de la propuesta de Caroline Bersot (`propuesta-cse-v1`, tal como quedó
 * el 2026-09-02) con tres diferencias, todas pedidas por Elías el 2026-09-22:
 *
 *   1. LA OFERTA BAJA: $1,000.00 quincenal / $2,000.00 mensual (Caro: $1,250 / $2,500),
 *      y el salto del mes 3 va a $2,200.00 mensuales (Caro: $2,600.00). Las dos
 *      comisiones —expansión 5/2 ó 3/4 y el 10% de licencias Collab— quedan IGUALES.
 *   2. SIN BLOQUES DE FIRMA: la de Caro cierra con dos espacios de firma fechados el
 *      2 de setiembre. Acá no se necesitan todavía.
 ...
 * haya editado acá desde la pantalla.
 *
 * Uso:
 *   dry-run:  npx tsx scripts/seed-propuesta-cse-valentina.ts
 *   aplicar:  $env:ALLOW_PROD_WRITE="1"; npx tsx scripts/seed-propuesta-cse-valentina.ts --apply
 */
...
const CONTENT = {
  ...
  // ── La oferta ─────────────────────────────────────────────────────────────
  // DIFERENCIAS con la de Caro: el base baja a $1,000 quincenal / $2,000 mensual,
  // el salto del mes 3 va a $2,200 mensuales, entra la fecha de ingreso y salen
  // los dos bloques de firma.
  oferta: {
    ...
    filas: [{ concepto: "Customer Success Executive", quincenal: "$1,000.00", mensual: "$2,000.00" }],
    destacados: [
      {
        titulo: "Plan de crecimiento (al finalizar el mes 3)",
        texto:
          "Al cerrar los 3 meses iniciales el salario base pasa a $2,200.00 mensuales ($1,100.00 quincenal). El hito viene con una revisión formal del desempeño, el impacto y la evolución del rol.",
      },
      {
    ...
    bloques: [
      {
        ...
        items: [
          "Fecha de ingreso: jueves 1 de octubre de 2026.",
...
async function main() {
  ...
  try {
    const previo = await prisma.roleProfile.findUnique({
      where: { id: ROLE_ID },
      select: { id: true, docType: true, publicToken: true },
    });
    ...
    console.log(`  oferta:  ${of.filas.map((f) => `${f.quincenal} quincenal / ${f.mensual} mensual`).join(" · ")}`);
    console.log(`  mes 3:   ${of.destacados[0].texto.match(/\$[\d,.]+ mensuales/)?.[0] ?? "—"}`);
    console.log(`  ingreso: ${of.bloques[0].items[0]}`);
    console.log(`  firmas:  ninguna (a diferencia de la de Caro)`);

    if (previo && previo.docType !== "PROPUESTA") {
      console.error(`⛔ ${ROLE_ID} existe y es ${previo.docType}, no PROPUESTA.`);
      process.exit(1);
    }
    if (previo?.publicToken) {
      console.log("  ⚠ este documento YA tiene link público publicado: el contenido que se sirve cambia al aplicar.");
    }

    if (!APPLY) {
      console.log("\nDRY-RUN. Para aplicar:");
      console.log('  $env:ALLOW_PROD_WRITE="1"; npx tsx scripts/seed-propuesta-cse-valentina.ts --apply');
      return;
    }
    ...
    console.log("\n✓ Listo. Abrí Roles y revisá la propuesta de Valentina.");
  } finally {
    await close();
  }
}
