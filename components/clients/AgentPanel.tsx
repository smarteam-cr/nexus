    ...
    <>
      ...
      {open && (
        <>
          ...
          <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[600px] max-h-[70vh] bg-white border border-gray-200 rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
            {/* Header */}
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100">
              <div>
                <h3 className="text-sm font-semibold text-gray-900">Agentes disponibles</h3>
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-2">
        ...
        <div>
          <h4 className="text-xs font-semibold text-gray-700 uppercase tracking-wide">{group.label}</h4>
          <p className="text-[10px] text-gray-400">{group.description}</p>
        </div>
      </div>

      {agents.length === 0 ? (
        <div className="px-3 py-2 rounded-lg bg-gray-50 border border-dashed border-gray-200">
            ...
            return (
              ...
              >
                <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${
                  isRunning ? "bg-amber-100 border border-amber-200" : "bg-brand/5 border border-brand/10"
                }`}>
                  {isRunning ? (
