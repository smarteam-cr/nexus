...
/**
 * ROLES (perfiles de puesto y propuestas): quien ADMINISTRA la sección — dirección y el
 * **CSL** (`esAdminDeRoles`, 2026-09-23). Gate hardcodeado fuera de la matriz de permisos
 * (mismo criterio que Costos: no se delega por plantilla). PRIMERA línea de TODO handler
 * bajo /api/roles.
 *
 * ⚠ NO lleva el id del documento, y eso SOLO es correcto mientras quien pasa este guard vea
 * TODOS los documentos —que es justo lo que garantiza `esAdminDeRoles` en `visibleRoleWhere`,
 * la misma función—. Si alguna vez entra acá un rol con visibilidad PARCIAL, este guard deja
 * de alcanzar: estos handlers toman el id de la URL, así que habría que verificar además
 * `canReadRoleDoc(subject, id)` o ese rol administraría documentos ajenos probando ids. Las
 * dos mitades tienen que moverse juntas.
 */
