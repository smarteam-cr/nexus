/**
 * lib/sessions/cargar-sesiones-categorizadas.ts — TODAS las sesiones, categorizadas, para /sessions.
 *
 * C-20 (2026-09-04): extraído de app/(shell)/sessions/page.tsx sin cambiar una consulta, para que
 * la página (al entrar) y GET /api/sessions/grupo (al cambiar de grupo) carguen por el MISMO
 * camino: misma cascada, mismos flags, misma cobertura. Un grupo que se pide por la ruta tiene
 * que ser exactamente el que la barra contó.
 *
 * ⚠ SIN corte por fecha: las reuniones futuras SE MARCAN, no se esconden (decisión CTX1.2). El
 * censo de lectores de lib/sessions/ocurridas.test.ts lo clasifica como «agenda» por eso.
 */
...
export type SesionesCategorizadas = Awaited<ReturnType<typeof cargarSesionesCategorizadas>>;
export type SesionCategorizada = SesionesCategorizadas["sessionsWithMeta"][number];

export async function cargarSesionesCategorizadas() {
  // ── 1. Cargar data base ────────────────────────────────────────────────────
